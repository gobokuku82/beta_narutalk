...
| 담당자 | 도윤 |
| 분류 | 개발 - AI 에이전트 |
| 진행상태 | 작업중 (Sprint 13 Integration 반영) |
| 버전 | **v1.5** |
| 최종 수정일 | 2026-04-22 |
...
>
> **v1.3 What's New** (Sprint 12 — 이전 버전)
> - `execution_progress` 필드 신규 — HITL Pause/Resume 영속화
> - v2 실제 사용 필드 정리 (`structured_query`, `plan`, `execution_result`, `response`)
> - Sprint 13 예정 필드 명시 (예고)
> - v1 잔재 필드 deprecated 표시
...
## 2. 스키마 (v1.4 — Sprint 13 Integration 반영)

### 2.0 현행 (v2 코드 기준, Sprint 13 Integration I6~I10f 완료)

| 필드 | 타입 | Reducer | 초기값 | Pydantic 모델 | 없을 때의 의미 | 설명 |
|------|------|---------|--------|---------------|-------------|------|
| **`user_id`** | `str` | - | Settings fallback ("demo") | - | - | **✅ Sprint 13 I6** — 사용자 식별. Multi-user 대비 |
| **`conversation_id`** | `str` | - | (필수) | - | - | **✅ Sprint 13 I6** — 대화방 단위 (UI 채팅방 1 = 1 conversation) |
| **`turn_id`** | `str` | - | (필수) | - | - | **✅ Sprint 13 I6** — 단일 쿼리 단위 (대화 안 1턴) |
| `session_id` | `str` | - | `turn_id` alias | - | `turn_id`와 동일 | **deprecated**. 내부 변수용 alias만 유지. 외부 계약은 `turn_id` |
| **`conversation_history`** | `list[dict]` | - (LWW) | `[]` | - | 이전 턴 기록 없음 | **✅ Sprint 13 I6/I8** — Cognitive 주입용 최근 N턴 요약 (Sprint 15에서 MemoryManager가 채움) |
| **`history_limit`** | `Optional[int]` | - | Settings fallback | - | 전체 포함 | **✅ Sprint 13 I6** — conversation_history 주입 제한 턴 수 |
| `user_input` | `str` | - | (필수) | - | - | 사용자 원본 입력 |
| `language` | `str` | - | `"ko"` | - | 한국어 기본 | 언어 코드 (ko/en/ja) |
| **`client_id`** | `str` | - | payload 명시 시 set, 미명시 시 키 자체 absent (total=False) | - | ExecutionContext.client_id = None → BaseTool.fetch fail-fast | **✅ 작업 ⑪ (2026-05-31)** — ADR-022 helper-B agent path 활성화. Writer=ws_agent (init_agent_state via payload) · Reader=execution_stage (→ExecutionContext). 접근 컨벤션 = `state.get("client_id")` (직접 인덱싱 금지) |
...
| `execution_progress` | `dict[str, Any]` | - (LWW) | `{}` | `ExecutionProgress` (dataclass) | Pause/Resume 정보 없음 | Sprint 12 — Phase별 진행 상태 + completed_todos. Checkpoint 영속화 |
...
### 2.0-a Sprint 13 Session/Thread 식별 체계 (구현 완료)

| 용어 | 계약 | 예시 | 사용 위치 |
|------|------|------|-----------|
| `user_id` | 사용자 식별자 (현재 "demo" 고정) | `"demo"` | `/ws/agent?user_id=...`, state, broadcast fan-out |
| `conversation_id` | 대화방 UUID (클라이언트 생성) | `"conv_a1b2c3d4"` | query 메시지, state |
| `turn_id` | 쿼리 UUID (클라이언트 생성, 매 쿼리마다) | `"turn_e5f6g7h8"` | query 메시지, state, HITL signal_resume 키 |
| `thread_id` | LangGraph Checkpointer 키 = `f"{conversation_id}_{turn_id}"` | `"conv_a1b2c3d4_turn_e5f6g7h8"` | `config["configurable"]["thread_id"]` |
...
> ⚠️ **2026-05-15 정정 노트**: 본 §의 "TodoItem" 표현은 v1.5 시점의 deprecated `models.TodoItem` 을 가리켰음. **활성 모델은 `planner.PlannedTodo`** (`backend/app/dream_agent/planning/planner.py`) — ADR-010 단일화 후 `models/todo.py` 삭제 (2026-05-15 cleanup A4). `agent` 필드는 이미 `PlannedTodo` 에 정식 포함.
> 상세 필드 정의는 [`30_DATA_MODELS_v1.1.md`](30_DATA_MODELS_v1.1.md) §2.2 참조.
> 본 §11 spec 자체의 `plan` 구조(`plan_id`/`intent_summary`/`dependency_graph`/`strategy`/`mermaid_diagram`/`visualization`)도 deprecated `models.Plan` 의 옛 구조 — 활성 `planner.Plan` 은 `{teams_selected, todos, dag, plan_notes}` 4필드. **v1.6 bump 시 §"Planning 산출" 전면 재작성 필요** (별도 트랙).
...
## 3. Reader/Writer 매트릭스 (v1.4 — Sprint 13 Integration 반영)

| 필드 | cognitive | planning | execution | response | hitl_manager | ws_agent | 비고 |
|------|:-:|:-:|:-:|:-:|:-:|:-:|------|
| **`user_id`** | R | R | R | R | - | **W** | ✅ Sprint 13 I6 — init_agent_state에서 설정 |
| **`conversation_id`** | R | R | R | R | - | **W** | ✅ Sprint 13 I6 — init_agent_state에서 설정 |
| **`turn_id`** | R | R | R | R | R | **W** | ✅ Sprint 13 I6 — HITL signal_resume 키 |
| `session_id` (deprecated) | R | R | R | R | R | W | = turn_id alias (내부 호환만) |
| **`conversation_history`** | **R** | - | - | - | - | **W** | ✅ Sprint 13 I6/I8 — Cognitive 주입용 (Sprint 15 MemoryManager가 채움) |
| **`history_limit`** | R | - | - | - | - | **W** | ✅ Sprint 13 I6 — Settings fallback |
| `user_input` | R | - | - | - | - | **W** | Cognitive만 사용 |
| `language` | R | - | - | R | - | **W** | 다국어 지원용 |
...
| `error` | W | W | W | W | W | W | 모든 레이어에서 에러 발생 가능 |
| `hitl_pending` | - | (interrupt) | (interrupt) | - | R | - | interrupt() payload (graph_state.tasks에서 추출) |
| `trace` | W | W | W | W | - | - | 모든 레이어에서 로그 추가 |

**Sprint 13 Integration 변화**:
- Sprint 13 식별 필드(`user_id`/`conversation_id`/`turn_id`) 및 `conversation_history`/`history_limit` 추가 — Writer는 ws_agent (`init_agent_state` 호출 시점)
- `ws_agent` 컬럼 신설 — state 초기화 시 필드 주입 책임 명확화
- `session_id`는 deprecated alias로 표시 (Writer는 `init_agent_state` 내부 동기화만)
...
> ⚠️ **현재 상태 (Sprint 13 말)**: 아래 reducer들은 **설계 목표이며 아직 구현되지 않음**. 
> `agent_state.py` 는 순수 `TypedDict` 로 LangGraph 기본 동작(last-writer-wins) 사용 중. 
> POC → MVP 전환 시 `backend/app/dream_agent/states/reducers.py` 로 구현 예정 (Sprint 14+).
>
> 현재 동작 가능한 이유: 각 Layer 가 Command(update={...}) 로 배타적 필드만 업데이트하여 
> 충돌 경로가 실질적으로 없음. 다중 Writer 필드(`trace`, `todos`, `execution_results`)도 
> 현재는 단일 경로로만 쓰이고 있어 실제 충돌 미발생. 단 **여러 노드에서 append 할 시점이 오면 반드시 reducer 필요**.
...
## 8. 초기 State 값 (v1.4 — Sprint 13 Integration 기준)

`init_agent_state(...)` 호출 결과 (`backend/app/dream_agent/states/agent_state.py`):
...
def init_agent_state(
    *,
    ...
    """Sprint 13 표준 초기화. user_id/history_limit은 Settings fallback."""
    if user_id is None:
    ...
    return {
        ...
        "turn_id": turn_id,
        "session_id": turn_id,                 # deprecated alias 동기화
        ...
        # 횡단 — trace만 초기화 (append-only 대비)
        "trace": [],
    }
```

**설계 결정 — 왜 산출물 필드를 초기화 안 하는가?**:
- `AgentState(TypedDict, total=False)` — 모든 필드 optional
- 각 레이어가 자기 필드를 **처음 쓸 때** 나타나는 방식 (LangGraph 부분 업데이트 최적화)
- `structured_query`/`plan`/`execution_result`/`execution_progress`/`response`/`error`/`hitl_pending`은 **초기 dict에 없음** — `state.get("structured_query", {})` 패턴으로 읽기

**Sprint 13 관련 Settings 필드** (`backend/app/core/config.py` — T5 전체 7필드):

| Settings | 기본값 | 용도 | 사용 시점 |
|----------|-------|------|-----------|
| `DEFAULT_USER_ID` | `"demo"` | user_id fallback (Sprint 16+ 실제 로그인 전) | `init_agent_state`, `/ws/agent?user_id` 미지정 시 |
| `DEFAULT_HISTORY_LIMIT` | `3` | history_limit fallback | `init_agent_state` |
...
| Settings | 기본값 | 용도 |
|----------|-------|------|
| `CHECKPOINT_DB_URI` | `"postgresql://..."` | AsyncPostgresSaver URI (lifespan) |
| `HITL_TIMEOUT_SEC` | `300` | Sprint 12 legacy `_run_agent`의 wait_for_response 타임아웃 (run_turn은 timeout 없음 — Sprint 14에서 도입 예정) |

**Sprint 13 변화 (v1.3 대비)**:
- `create_initial_state` → `init_agent_state` (명명 변경)
- `session_id` 단독 필수 → `turn_id` 필수 + `session_id` 자동 alias
- `user_id`/`conversation_id`/`history_limit`/`conversation_history` 필드 추가 (Settings fallback)
- v1 잔재 필드(`cognitive_result`/`planning_result`/`execution_results`/`response_result`/`todos`) 초기화 제거
- 산출물 필드(`structured_query`/`plan`/`...`) 초기화 **제거** — optional TypedDict 특성 활용

**호출 경로**:
- `ws_agent._graph_runner_with_resume` 진입 시 1회 호출
- 테스트: `from app.dream_agent.states.agent_state import init_agent_state` 직접 사용
