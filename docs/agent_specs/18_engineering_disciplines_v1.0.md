...
관련 spec: [14 System Agent Overview](14_system_agent_overview_v1.0.md)(4-Layer 책임) · [16 Layer Dependency](16_layer_dependency_architecture_v1.0.md)(물리 의존) · [37 PMAL](37_agent_language_pmal_v1.0.md)(에이전트 언어) · [39 Query Categories & Routing](39_query_categories_and_routing_v1.0.md)(라우팅).
...
| **tool 구현** | **도구 실행 로직** | *실제 계산·분석·수집이 맞나* — 순수 기능, produces/consumes 계약 | `tools/{collection,metrics,analysis,qa,decision,...}/` · `ml_models/` |
...
게이트는 **한 덩어리가 아니다.** 두 종류이고 운명이 다르다 ([cp1 진단](../_claude/4layer_system/) 박제):
...
| **enforce_breakdown_dimension** | `planner.py:445` | operation=breakdown+dimensions 인데 차원분해(rows) tool 누락 시 차원대응 tool 삽입. convention=produces∋rows + name.startswith(dim)(하드코딩 맵 없음, channel→channel_aggregate). "채널별 ROAS" scalar 붕괴 해소(2026-06-11 ⒠) |
| **bind_temporal_params** | `planner.py:274` | period / period_a·b(MoM) 결정론 바인딩 |
| **apply_subject_coherence_filter** | `planner.py:152` | 텍스트 의도 없으면 리뷰-데이터 tool 제외(F2 잠정 가드) |
...
복합쿼리 깨짐 5종을 4축으로 귀속(워크플로우 6 추적기 + 적대검증, [근원귀속 박제](../_claude/4layer_system/stage2_rootcause_attribution_260611.md)):
...
| **revenue_total period drift** | **tool 구현** | catalog `params_required=[]` vs 코드 period 필수(유일 실측 tool 버그, ⒜ 정합) |
| **silent-0 빈출력** | **하네스** | LLMTool degrade 가 status 전파 안 함(⒞ 해소) |
| **breakdown 차원 누락**(채널별→scalar ~60%) | **프롬프트 → 하네스 해소** | Stage3 가 per-channel tool 대신 전체 scalar 선택 → 게이트 ⒠(enforce_breakdown_dimension) 결정론 보강 |

### 무게중심: **프롬프트 > 하네스 >> tool 버그. tool·데이터는 주범 아님.**
- **tool 미구현(stub 18)** = 의도된 POC mock, plan 안 깨뜨림. 복합쿼리 핵심 tool 전부 구현.
- **tool 구현버그** = 1건·국소(revenue_total drift). roas_overall drift=거짓, format_normalizer=과장.
- **데이터 레이어**(data_sources/workspace/schemas) = **건강**(4월 정답 재현). "5월 0"=mock raw 시간 커버리지(≤2026-04) 부재이지 전달버그 아님. ([데이터 검증](../_claude/4layer_system/))
- **적대검증이 1차 오진 2건 기각**: "DAG cycle(역방향 간선)"·"cognitive validation 실패" = 허구(코드 자기일관·`extra='forbid'` 아님).
- **★ 복합 multi-run 베이스라인**(2026-06-11, 5런×14쿼리, `baseline_compound.py`): systematic 깨짐 **0** · lv4/lv5(의존체인·fan-out·산출) **100%** → 깨짐은 좁은 tool-선택 비결정뿐. **R2(sub_intents 본배선)·broad Stage3 재작성 둘 다 데이터 미정당**(고칠 결정론 gap 없음·안정 다수 회귀 위험) → 좁은 게이트 ⒠로 93.5%→**98.5%**(σ22→8.5%). 단발 score_coverage 는 1회 LLM 스냅샷(judge 포함 3중 비결정)이라 **multi-run 평균±안정성 분류(stable/systematic/flaky)**가 ⒝/R2 선택의 근거·회귀 기준선. ([findings](../_claude/4layer_system/baseline_compound_findings_260611.md))

> **방법론적 교훈**: 추적기 1차를 코드 실행 없이 단정하면 오진. **catalog≠code 실측 + 적대검증**이 근원귀속의 필수 절차. ([[project_catalog_code_drift]])
...
| breakdown 쿼리가 차원 무시(채널별→전체) | **프롬프트(Stage3 tool선택) → 하네스 게이트** | operation=breakdown 인데 plan 에 rows 산출 tool 있나? 없으면 게이트 ⒠ 발동(auto_ id 확인) |
| 빈 답·"분석 완료"만 | **하네스(silent-0)** or **데이터 부재** | exec todos SKIPPED? data_insufficient? raw 커버리지 확인 |
| 자신만만한 *틀린* 답(리뷰로 샘) | **컨텍스트(PMAL domain 손실)** or **프롬프트(cognitive)** | intent.domain 보존됐나? F2 |
...
진단 절차: **워크플로우 병렬 추적(축별) → 적대검증(오귀속 색출) → 합성.** ([근원귀속 박제](../_claude/4layer_system/stage2_rootcause_attribution_260611.md) §방법.)
...
| 버전/일자 | 내용 |
|---|---|
| v1.0 — 2026-06-11 | 신설. 사용자 프레임(8gate=하네스/4-Layer=컨텍스트/프롬프트=프롬프트 + tool 구현 4축) 박제 + stage-2 근원귀속(워크플로우 6추적+적대검증) 흡수. §2 게이트 두 종류(현실가드/가리개), §3 깨짐→축 매트릭스(프롬프트>하네스>>tool, 데이터 건강), §4 징후→진단 가이드. 짝 자취 = `_claude/4layer_system/stage2_rootcause_attribution_260611.md`. |
| v1.1 — 2026-06-11 | "측정 먼저" 흡수. §2 ⓑ 게이트 **enforce_breakdown_dimension(⒠, planner.py:445)** 추가 — breakdown 차원 누락 결정론 보강. §3 매트릭스에 breakdown 행 + **복합 multi-run 베이스라인**(systematic 0·R2 미정당·93.5→98.5%) 박제. §4 진단 가이드에 baseline_compound.py(stable/systematic/flaky 분류) 추가. 짝 자취 = `_claude/4layer_system/baseline_compound_findings_260611.md`. |
