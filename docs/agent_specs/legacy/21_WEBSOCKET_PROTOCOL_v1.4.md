...
| 담당자 | 도윤 |
| 분류 | 개발 - API 계약 |
| 진행상태 | **Active** (Sprint 13 Integration I6~I11-a 반영) |
| 버전 | **v1.4** |
| 최종 수정일 | 2026-04-24 |
| 이전 버전 | v1.3 (2026-04-23), v1.2 (2026-04-22), `POC_legacy/WEBSOCKET_PROTOCOL_poc.md` (v1.1, 2026-04-10) |
| 관련 명세 | `20_INTERFACE_CONTRACT_v1.1.md`, `22_error_codes_v1.1.md`, `24_sequence_diagrams_v1.3.md`, `30_DATA_MODELS_v1.1.md`, `10_system_architecture_v1.9.md`, `12_manager_layer_v1.3.md` |
...
서버는 `user_id` 기준으로 여러 WebSocket을 관리 (탭 다중 지원):
- **MAX_WS_CONNECTIONS_PER_USER = 5** — 초과 시 close(1008) `connection_limit_exceeded`
- **broadcast fan-out**: 한 `user_id` 의 모든 연결에 이벤트 동일 전송
- **dead WS 자동 정리**: 송신 실패 시 해당 연결 제거

상세: `10_system_architecture_v1.9.md` §4.2 (Connection Manager)
...
| 용어 | 생성 | 용도 |
|------|------|------|
| `user_id` | URL query | 사용자 식별 + broadcast fan-out 키 |
...
**옵션**: `language` (기본 "ko"), `conversation_history`, `history_limit` (Settings fallback)
...
| Layer Guard | `backend/api_v2/layer_guard.py` (I11-a) |
