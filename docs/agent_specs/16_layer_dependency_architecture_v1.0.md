...
> **결정 근거**: DataSource/Workspace 분리, 폴더 명명 원칙.
...
**(A) 레이어 스택** — 의존은 *하향*만:
```
 top entry      backend/api/                 ← 아무도 역참조 안 함 (HTTP/WS route)
                      │ (down)
 ...
 agent data     app/dream_agent/{models, schemas}   (순수 계약)
                      │ (down)
 data layer     app/data_layer/{data_sources, workspace, schemas}
```
...
| # | 불변식 | 재현 grep | 결과 |
|---|---|---|---|
| **I1** | data layer는 agent를 모른다 | `app\.dream_agent` in `app/data_layer/{data_sources,workspace,schemas}` | **0건** (전부 import도 docstring도 0) |
...
| **I4** | orchestration stage는 DAG | 역방향(cognitive→response, planning→execution) import | **0** — 코드 전체 import 순환 **0** (구 V1 순환 2026-06-05 해결) |

→ 핵심 의도(`api → agent → tool → data`)의 두 불변식(I1·I2)이 **완벽히 holds**. 의외로 깨끗한 레이어드다.
...
| 디렉터리 | 분류 | 책임 | 비고 |
|---|---|---|---|
| `backend/api/` | 스택: top entry | HTTP/WS route | app 밖. 아무도 역참조 X |
...
| `app/data_layer/{data_sources,workspace,schemas}` | 스택: data layer | INPUT 관절 / OUTPUT 저장 / 컬럼 계약 | agent 무지 |
| `app/core` · `llm_manager` · `workflow_managers` · `states`·`system_graph` | **cross-cutting 서비스/조립** | logging·config / LLM클라 / HITL·세션·콜백·learning | 스택 밖 — layer가 부르고 서로 부름(§1B). 순환만 금지 |
...
| 🔵 개선 | **V3** | LLM 사용 tool → `llm_manager.client`. llm_manager=cross-cutting 서비스라 **하드 위반 아님**. 단 **"tool=순수기능"** 원칙상 LLM client 주입이 더 깨끗 | `tools/llm_tool.py` | DI(ExecutionContext 주입). **진행 승인** |
...
**정리**: 스택-방향 하드 결합은 V1(순환)·V5(stage↔stage) **둘뿐 — 모두 ✅ 해결**. V3는 cross-cutting 서비스 호출이라 하드 위반은 아니나 **tool 순수성·테스트성**을 위한 DI 개선 대상(승인·진행). V4는 매니저 cross-cutting이라 정상. **전부 작동 영향 0.**

---

## §5 tool 데이터 접근 규칙 (순수성 경계)

- **유일 진입점**: `BaseTool.fetch(source_id, context)` → `self.ds.get(client, source_id)` ([base_tool.py](../../backend/app/dream_agent/tools/base_tool.py)). client는 `ExecutionContext.client_id`에서만 흐르고 없으면 fail-fast.
- **직접 파일 I/O 예외**: `tools/registry.py`(catalog YAML=tool 메타).
- `data_layer.schemas.inputs.load_*()`는 fetch한 DataFrame을 파싱하는 **순수 함수** → 직접 조회 누수 아님.
...
| spec 10 §7.7.2 (Tool DI 예시) | **폐기 패턴** | `client` default → 실제 `self.fetch+context.client_id`(§5) |
| DataSource/Workspace 분리 결정 | current | §1 data layer 근거 |
| 폴더 명명 원칙 | partially-stale | 결정한 `normalizers/` 폴더 부재 → 이력 박제 필요 |

**후속(별도)**: spec 10 §7.7.2 DI 예시 교체.
...
```
# I1: 0
rg "app\.dream_agent" backend/app/data_layer/data_sources backend/app/data_layer/workspace backend/app/data_layer/schemas
...
# V3 (개선 대상): cross-cutting 서비스 직접 import
rg "llm_manager" backend/app/dream_agent/tools                       # V3
```
...
> 백엔드는 `api→agent→tool→data` **스택 하향**을 잘 지키고(I1·I2 grep 0), 그 위에 **cross-cutting 서비스/매니저**(core·llm_manager·workflow_managers)가 가로지른다(스택 밖, 순환만 금지). 스택-방향 하드 결합 V1(순환)·V5(stage↔stage)는 ✅ 해결. V3는 하드 위반 아니나 tool 순수성·테스트성 위해 DI 개선(진행). V4는 매니저 cross-cutting이라 정상(정정). 본 문서가 의존-방향 단일 권위(재현 §7).
