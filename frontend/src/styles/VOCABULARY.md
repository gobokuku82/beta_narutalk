...
| **Track** | **트랙** | 보드 안의 큰 구분 (그룹화된 카드 묶음) | MonthlyPage 의 "01 성과" / "02 마케팅" 등 |
| **Section** | **섹션** | Track 대체 명칭 — 일반 페이지에서 (분석 5 페이지 등) |  |
| **Strip** | **스트립** | 가로 한 줄에 dense 하게 배치된 cell 들 | `MetricChainStrip` · `MomBar` · `MemberGuestSummary` |
| **Cell** | **셀** | Strip 안의 개별 단위 (Card 보다 작음) | `MemberGuestSummary` 의 4 cell |
| **Frame** | **프레임** | 차트의 셸 (제목 + 차트 컨테이너) | `ChartFrame` |
| **Chain** | **체인** | 인과 관계로 묶인 메트릭 사슬 | `MetricChainStrip` (마케팅비→매출→AOV) |
...
| **Welcome Hero** | **웰컴 히어로** | 첫 진입 화면 상단 강조 섹션. 시스템 정체성 (Brand 워드마크 + 페르소나 한국어 + 한 줄 가치). 좌측 옥스블러드 strip (PALETTE J) 1군데. 차분한 결 (Apple/Stripe 결) | `WelcomeHero.tsx` |
...
| **Persona Card** | **페르소나 카드** | 멀티 페르소나 비전 (Data Analyst·Campaign Designer·Media Director...). 1 활성 (좌측 옥스블러드 strip + foreground) + N 예정 (dimmed + "예정" 라벨) 카드 그리드 | `PersonaCards.tsx` |
...
### 5.1 외곽 카드 hover (Card · ChartFrame · MetricChainStrip · ChannelPanelCard · PacingRow)
...
| **H3** | hover 시 `bg-primary/4` (옅은 옥스블러드 4% tint) |
...
| **H7** | **Stage (FunnelChart bar 같은 직접 viz 단위)** hover = `opacity 0.80 → 1.0` + `ring-2 ring-primary/40` (bar 자체 색 진해짐 + 외곽선). 마케팅 5매체 recharts 자동 hover 와 정신 동등. 2026-06-10 v2 신설 |
...
| `components/viz/ChannelComparison.tsx` ChannelPanelCard | H1+H2+H3+H4 | 4 채널 카드 |
| `components/viz/PacingWidget.tsx` (각 campaign row) | H1+H2+H3+H4 | row = 카드 결 |
...
| `features/monthly/MemberGuestSummary.tsx` Cell | H5 (Cell) | grid 안 4 cell |

## 6. 자취

- **2026-06-10 v1** : 보드/카드 어휘 박제 + Hover H1~H5 룰 (강화: ring + bg tint + -translate-y-0.5). Cell hover "미적용".
- **2026-06-10 v2** : H2 lift 강화 (2px → 4px). H5 Cell hover 재정의 (미적용 → bg-primary/8 절제 적용). H7 Stage hover 신설 (FunnelChart 등 직접 viz). 적용 매트릭스 확장 (ChannelPanelCard / PacingWidget row / MomBar / MemberGuestSummary).
...
- **2026-06-10 v5** : H5 Cell hover bg 제거 — bg-primary/10 → 없음. 옥스블러드 ring-inset 단일 신호. 미니멀 (사용자 결정).
...
- **2026-06-10 v10** : Persona Tab / Feature Showcase / Showcase Media 어휘 신설 — Header 재구성 (시계 제거, DreamAgent 큰 워드마크 + Data Analyst/Campaign Designer/Media Director 페르소나 탭). FeatureShowcase 신설 (데이터 표시·회의록 그래프·에이전트 워크플로우 3 카드, inline SVG 자동 애니메이션, shadow-lg ring-1 음영 강조). 향후 GIF/MP4 미디어 교체 자리 마련.
...
- **2026-05-22** (이전, 별도) : viz/* 컴포넌트 명명 — ChartFrame, MetricChainStrip, DataTable, MemberGuestSummary 등 (도메인 명사 위주)
