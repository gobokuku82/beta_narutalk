...
{{
    ...
    async def use_calculation_tool(
        self,
        state: Dict[str, Any],
        results: Dict[str, Any],
        runtime: Runtime[AgentContext]
    ) -> Dict[str, Any]:
        """Use calculation tool for metrics"""
        try:
            tool = self.tools["calculation"]
            tool_results = {}

            # Get data from results
            collected_data = results.get("collected_data", {})

            # Calculate achievement rate if we have performance and target data
            if collected_data.get("aggregated_performance") and collected_data.get("aggregated_target"):
                perf = collected_data["aggregated_performance"].get("monthly_totals", {})
                target = collected_data["aggregated_target"].get("monthly_targets", {})

                # Calculate for each month
                achievement_rates = {}
                for month in perf:
                    if month in target:
                        rate = tool.calculate_achievement_rate(perf[month], target[month])
                        achievement_rates[month] = rate

                tool_results["achievement_rates"] = achievement_rates
                tool_results["average_achievement"] = tool.calculate_average(list(achievement_rates.values()))

            # Calculate growth rate if we have historical data
            if collected_data.get("aggregated_performance"):
                monthly = collected_data["aggregated_performance"].get("monthly_totals", {})
                if monthly:
                    mom_growth = tool.calculate_mom_growth(monthly)
                    tool_results["growth_analysis"] = mom_growth

            return tool_results

        except Exception as e:
            self.logger.error(f"Error using calculation tool: {e}")
            return {"error": str(e)}

    async def use_trend_tool(
        self,
        state: Dict[str, Any],
        results: Dict[str, Any],
        runtime: Runtime[AgentContext]
        ...
        try:
            ...
            if collected_data.get("aggregated_performance"):
                ...
                if monthly:
                    ...
                    trend = tool.analyze_historical_trend(values, labels)
                    tool_results["trend_analysis"] = trend

                    # Detect seasonality
                    seasonality = tool.detect_seasonality(monthly)
                    tool_results["seasonality"] = seasonality

                    # Make predictions
                    if len(values) >= 3:
                        predictions = tool.predict_future_trend(values, periods_ahead=3)
                        tool_results["predictions"] = predictions

            return tool_results

        except Exception as e:
            self.logger.error(f"Error using trend tool: {e}")
            return {"error": str(e)}

    async def use_cross_db_tool(
        self,
        state: Dict[str, Any],
        runtime: Runtime[AgentContext]
    ) -> Dict[str, Any]:
        """Use cross-database analysis tool"""
        try:
            tool = self.tools["cross_db"]
            query = getattr(runtime.context, "original_query", "")

            # Extract parameters from query
            parsed = state.get("parsed_query", {})
            person_name = parsed.get("person_name") or parsed.get("name")

            if person_name:
                # Analyze personal performance
                result = await tool.analyze_personal_performance(person_name)
                return result
            else:
                # General analysis
                return {"message": "Cross-DB analysis requires specific parameters"}

        except Exception as e:
            self.logger.error(f"Error using cross-db tool: {e}")
            return {"error": str(e)}
