...
class SalesAnalyticsAgent:
    ...
    def _build_graph(self):
        ...
        # Add nodes - all follow (state: SalesState, config: Dict[str, Any]) signature
    ...
    # ================ Core Node Functions ================
    # All nodes follow the pattern: (state: SalesState, config: Dict[str, Any]) -> Dict
    ...
    async def plan_execution(
        ...
        Args:
            ...
            config: Configuration dict with context
        ...
        try:
            ...
            original_query = config.get("configurable", {}).get("context", {}).get("original_query", query)
    ...
    async def execute_plan(
        ...
        try:
            ...
            if "data_collection" in plan.get("use_subgraphs", []):
                ...
                collection_result = await self._invoke_data_collection(state, config)
                results["collection"] = collection_result
            
            if "analysis" in plan.get("use_subgraphs", []):
                # Invoke analysis subgraph
                analysis_result = await self._invoke_analysis(state, config, plan)
    ...
    async def format_results(
        ...
        try:
            ...
            language = config.get("configurable", {}).get("context", {}).get("language", "ko")
    ...
    async def _invoke_data_collection(
        ...
        try:
            ...
            subgraph_context = filter_context_for_subgraph(
                dict(config.get("configurable", {}).get("context", {})),
                self.agent_name
            )
...
"""
    ...
    async def _execute_sql(self, state: SalesState, config: Dict[str, Any]) -> Dict[str, Any]:
        """Execute SQL query"""
